      ...
      <div className="container mx-auto px-4 max-w-6xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 relative z-10">
            {steps.map((step) => (
              <div key={step.number} className="bg-white p-6 rounded-xl shadow-md border border-gray-100 hover:shadow-lg transition-all relative">
                ...
                {/* Show description only when expanded - larger text */}
                {expandedSteps.includes(step.number) && (
                  <p className="text-gray-600 text-center text-base mb-3 transition-all">
                    {step.description}
                  </p>
                )}
                
                {/* Larger Read more / Read less button */}
                <button 
                  onClick={() => toggleDescription(step.number)}
                  className="text-blue-600 text-sm font-medium hover:text-blue-800 transition-colors focus:outline-none flex items-center justify-center mx-auto mt-2"
                >
                  {expandedSteps.includes(step.number) ? (
                    <>
                      Hide details
        ...
        <div className="mt-16 grid md:grid-cols-3 gap-8 text-center">
          <div className="bg-white p-8 rounded-xl shadow-sm">
            ...
            <p className="text-gray-700 mt-3">skill acquisition with structured learning paths</p>
          </div>
          <div className="bg-white p-8 rounded-xl shadow-sm">
            ...
            <p className="text-gray-700 mt-3">retention with our step-by-step approach</p>
          </div>
          <div className="bg-white p-8 rounded-xl shadow-sm">
            <div className="text-4xl font-bold text-blue-600 mb-2">92% <span className="text-blue-600 text-2xl font-semibold">Completion</span></div>
            <p className="text-gray-700 mt-3">rate compared to 15% for traditional online courses</p>
          </div>
        </div>
